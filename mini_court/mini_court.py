...
class MiniCourt():
    ...
    def convert_bounding_boxes_to_mini_court_coordinates(self,player_boxes, ball_boxes, original_court_key_points ):
        ...
        for frame_num, player_bbox in enumerate(player_boxes):
            ...
            # Eğer bu karede top yoksa → bir önceki geçerli top pozisyonunu kullan
            if ball_box is None:
                if frame_num == 0:
                    # İlk karede top yoksa sıfıra al (nadiren olur)
                    ball_box = [0, 0, 0, 0]
                else:
                    # En son geçerli top pozisyonunu al
                    ball_box = ball_boxes[frame_num - 1].get(1, [0, 0, 0, 0])
            ball_position = get_center_of_bbox(ball_box)

            # ✅ Top sahibini sadece 1 kez seç ve kilitle
            if last_ball_owner is None:
